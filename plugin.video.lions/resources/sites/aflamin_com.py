...
SITE_IDENTIFIER = 'aflamin_com'
SITE_NAME = 'Aflamin'
SITE_DESC = 'Moroccan drama catalogue'
...
def _getApiUrl():
    if not hasattr(_getApiUrl, 'url'):
        ...
        try:
            ...
            VSlog('aflamin_com: api url lookup failed (' + str(e) + ')')
...
def _apiQuery(sQuery, dVariables=None):
    ...
    oHeaders = {'User-Agent': 'Mozilla/5.0', 'accept-language': 'ar'}
...
def load():
    ...
    try:
        ...
        for oCat in aCategories:
            ...
            oGui.addDir(SITE_IDENTIFIER, 'showCategory', oCat['name'],
                        _getCategoryIcon(oCat['name']), oOutputParameterHandler)
    except Exception as e:
        VSlog('aflamin_com: load failed (' + str(e) + ')')
        oGui.addText(SITE_IDENTIFIER, '[COLOR red]Error loading catalogue[/COLOR]')
...
def showCategory():
    ...
    try:
        ...
        for oProduct in oProducts['items']:
            ...
            oGui.addDrama(SITE_IDENTIFIER, 'searchDrama', sLabel,
        ...
        if iPage < iLastPage:
            ...
            oGui.addDir(SITE_IDENTIFIER, 'showCategory', '[COLOR teal]Next >>>[/COLOR]',
                        icons + '/Next.png', oOutputParameterHandler)
    except Exception as e:
        VSlog('aflamin_com: showCategory failed (' + str(e) + ')')
        oGui.addText(SITE_IDENTIFIER, '[COLOR red]Error loading category[/COLOR]')
