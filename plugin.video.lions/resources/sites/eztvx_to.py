...
LOGO = 'special://home/addons/plugin.video.lions/resources/art/sites/eztvx_to.png'

SITE_IDENTIFIER = 'eztvx_to'
SITE_NAME = 'EZTV'
...
def __showTorrents(sHtmlContent):
    ...
    sPattern = r'<tr[^>]*class="forum_header_border"[^>]*>.*?<a href="(/ep/[^"]+)"[^>]*>([^<]+)</a>.*?<td[^>]*class="[^"]*"[^>]*>(\d+)</td>.*?<td[^>]*class="[^"]*"[^>]*>(\d+)</td>.*?<td[^>]*class="[^"]*"[^>]*>([^<]+)</td>'
...
def showHosters():
    ...
    if not magnetAdded:
        sPattern2 = r'href="(/torrent/[^"]+\.torrent)"'
