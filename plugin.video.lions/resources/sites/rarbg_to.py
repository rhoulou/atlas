...
LOGO = 'special://home/addons/plugin.video.lions/resources/art/sites/rarbg_to.png'

SITE_IDENTIFIER = 'rarbg_to'
SITE_NAME = 'RARBG'
...
def __showTorrents(sHtmlContent):
    ...
    sPattern = r'<tr[^>]*class="(?:even|odd)[^"]*"[^>]*>.*?<td[^>]*class="lista"[^>]*>.*?<a href="(/torrent/[^"]+)"[^>]*>([^<]+)</a>.*?</td>.*?<td[^>]*class="lista"[^>]*>([^<]+)</td>.*?<td[^>]*class="lista"[^>]*>(\d+)</td>.*?<td[^>]*class="lista"[^>]*>(\d+)</td>'
...
def showHosters():
    ...
    if not magnetAdded:
        sPattern2 = r'href="(/download/[^"]+\.torrent)"'
