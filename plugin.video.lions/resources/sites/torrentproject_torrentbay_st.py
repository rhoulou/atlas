...
LOGO = 'special://home/addons/plugin.video.lions/resources/art/sites/torrentproject_torrentbay_st.png'

SITE_IDENTIFIER = 'torrentproject_torrentbay_st'
SITE_NAME = 'TorrentProject'
SITE_DESC = 'TorrentProject torrent site on torrentbay.st'
...
def showSearch():
    ...
    if sSearchText:
        sUrl = URL_MAIN + '/search/' + urllib.parse.quote(sSearchText) + '/1/'
...
def __showTorrents(sHtmlContent):
    ...
    sPattern = r'<a href="/(?:torrent|detail)/(\d+)/?"[^>]*>([^<]+)</a>.*?<td[^>]*>([^<]+)</td>.*?<td[^>]*>(\d+)</td>'
