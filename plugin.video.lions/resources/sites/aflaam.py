...
LOGO = 'special://home/addons/plugin.video.lions/resources/sites/logos/aflaam.png'
UA = random_ua.get_ua()
...
def showMovies(sSearch = ''):
    ...
    sPattern = r'<a href="([^"]+)" class="box">[\s\S]*?data-src="([^"]+)" class="img-fluid w-100 lazy" alt="([^"]+)"'
...
def showMoviesSearch(sSearch = ''):
    ...
    sPattern = r'<a href="([^"]+)" class="box">[\s\S]*?<img src="([^"]+)" class="img-fluid[^"]*"[\s\S]*?<h3 class="[^"]*">([^<]+)</h3>'
...
def showEps():
    ...
    sPattern = r'<a href="([^<]+)">[\s\S]*?img src="([^"]+)" class="img-fluid w-100" alt="([^"]*)"[\s\S]*?class="font-size-50">([^<]+)<'
    aResult = oParser.parse(sHtmlContent, sPattern)
    ...
    sPattern = r'<a\s+href="([^"]+)"[\s\S]*?class="link-show d-flex align-items-center mx-2 ml-2">'
...
def showHosters():
    ...
    sPattern = r'<a\s+href="([^"]+)"[^>]*class="link-show d-flex align-items-center mx-2 ml-2">'
    aResult = oParser.parse(sHtmlContent, sPattern)
    if aResult[0]:
        murl = aResult[1][0]
        oRequest = cRequestHandler(murl)
        oRequest.addHeaderEntry('Referer', 'https://aflaam.com/')
        oRequest.setTimeout(TimeOut)
        sHtmlContent = oRequest.request()

    oParser = cParser()
    sPattern = r'<source[^>]*src="([^"]+)"[^>]*type="video/mp4"[^>]*size="([^"]+)"'
    aResult = oParser.parse(sHtmlContent, sPattern)
    ...
    if aResult[0]:
        for aEntry1 in aResult[1]:
            ...
            if oHoster != False:
                ...
                cHosterGui().showHoster(oGui, oHoster, sHosterUrl + "|verifypeer=false&Referer=https://aflaam.com/&User-Agent=" + UA, sThumb)
