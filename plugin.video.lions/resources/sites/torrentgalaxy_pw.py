...
LOGO = 'special://home/addons/plugin.video.lions/resources/art/sites/torrentgalaxy_pw.png'

SITE_IDENTIFIER = 'torrentgalaxy_pw'
SITE_NAME = 'TorrentGalaxy'
SITE_DESC = 'TorrentGalaxy torrent site'

URL_MAIN = siteManager().getUrlMain(SITE_IDENTIFIER)

CAT_FILMS = (URL_MAIN + '/torrents.php?cat=1', 'showFilms')
CAT_SERIES = (URL_MAIN + '/torrents.php?cat=5', 'showSeries')

URL_SEARCH = (URL_MAIN + '/torrents.php?search=', 'showSearch')
...
def showSearch(sSearchText=''):
    ...
    if sSearchText.startswith('http'):
        ...
        sUrl = URL_MAIN + '/torrents.php?search=' + urllib.parse.quote(sSearchText)
...
def __showTorrents(sHtmlContent):
    ...
    sPattern = r'<div class="tgx-torrent-list">.*?<tr>.*?<a href="(/torrents/[^"]+)"[^>]*>([^<]+)</a>.*?<span class="seeders">(\d+)</span>.*?<span class="leechers">(\d+)</span>.*?<span class="size">([^<]+)</span>'
