...
LOGO = 'special://home/addons/plugin.video.lions/resources/sites/logos/mycima.png'
...
MOVIE_ASIA = (URL_MAIN + '/category/%d8%a7%d9%81%d9%84%d8%a7%d9%85-%d8%a7%d8%b3%d9%8a%d9%88%d9%8a%d8%a9/', 'showMovies')
...
SERIE_TR = (URL_MAIN + '/category/%d9%85%d8%b3%d9%84%d8%b3%d9%84%d8%a7%d8%aa-%d8%aa%d8%b1%d9%83%d9%8a%d8%a9/', 'showSeries')
...
DOC_SERIES = (URL_MAIN + '/category/%d9%85%d8%b3%d9%84%d8%b3%d9%84%d8%a7%d8%aa-%d9%88%d8%ab%d8%a7%d8%a6%d9%82%d9%8a%d8%a9/', 'showSeries')
RAMADAN_SERIES = (URL_MAIN + '/category/%d9%85%d8%b3%d9%84%d8%b3%d9%84%d8%a7%d8%aa-%d8%b1%d9%85%d8%b6%d8%a7%d9%86-2026/', 'showSeries')
...
def load():
    ...
    oOutputParameterHandler.addParameter('siteUrl', 'http://venom/')
    oGui.addDir(SITE_IDENTIFIER, 'showSeriesSearch', 'SEARCH SERIES', LOGO, oOutputParameterHandler)

    oOutputParameterHandler.addParameter('siteUrl', 'http://venom/')
    oGui.addDir(SITE_IDENTIFIER, 'showAnimesSearch', 'SEARCH Animes', LOGO, oOutputParameterHandler)
...
def showMovies(sSearch = ''):
    ...
    sPattern = '<div class="Thumb--GridItem">\s*<a href="([^"]+)" title="([^"]+)">.*?data-lazy-style="--image: url\((.+?)\);".*?<span class="year">\(\s*(.+?)\s*\)</span>'
...
def showSeries(sSearch = ''):
    ...
    # (.+?) .+? ([^<]+)   
    sPattern = '<div class="Thumb--GridItem">\s*<a href="([^"]+)" title="([^"]+)">.*?data-lazy-style="--image: url\((.+?)\);">'
...
def showHosters():
    ...
    sPattern = r'<li\s+data-watch="([^"]+)"[^>]*>.*?<i class="fal fa-play"></i>(.*?)</li>'
    aResult = oParser.parse(sHtmlContent, sPattern)
    if aResult[0]:
        for aEntry in aResult[1]:
            sHosterUrl = aEntry[0] + '|Referer=' + URL_MAIN
            sTitle = sMovieTitle
    ...
    sPattern2 = r'href="([^"]+)"[^>]*><quality>([^<]+)</quality><resolution><i class="[^"]*"></i>([^<]+)</resolution>'
    aResult2 = oParser.parse(sHtmlContent, sPattern2)
    if aResult2[0]:
        for aEntry in aResult2[1]:
            sHosterUrl = aEntry[0] + '|Referer=' + URL_MAIN
            sTitle = sMovieTitle + ' [' + aEntry[2].strip() + ' ' + aEntry[1].strip() + ']'
