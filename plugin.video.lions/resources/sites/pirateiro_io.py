...
LOGO = 'special://home/addons/plugin.video.lions/resources/art/sites/pirateiro_io.png'

SITE_IDENTIFIER = 'pirateiro_io'
SITE_NAME = 'Pirateiro'
...
def __showTorrents(sHtmlContent):
    ...
    sPattern = r'<tr[^>]*>.*?<td[^>]*>.*?<a href="(/torrent/[^"]+)"[^>]*class="cellMainLink"[^>]*>([^<]+)</a>.*?</td>.*?<td[^>]*class="[^"]*nobr[^"]*"[^>]*>([^<]+)</td>.*?<td[^>]*class="green"[^>]*>(\d+)</td>.*?<td[^>]*class="red[^"]*"[^>]*>(\d+)</td>'
...
def __checkForNextPage(sHtmlContent):
    sPattern = r'<a class="turnoverButton[^"]*" href="([^"]+)"[^>]*>Next'
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
    if aResult[0]:
