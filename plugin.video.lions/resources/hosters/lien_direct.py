...
from resources.lib.util import urlEncode, Quote, Unquote
...
class cHoster(iHoster):
    ...
    def _getMediaLinkForGuest(self):
        ...
        if 'downet.net' in api_call:
            actual_url = api_call.split('|')[0] if '|' in api_call else api_call
            headers = {
                'User-Agent': 'Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.3894.32 Safari/537.36',
                'Referer': 'https://aflaam.com/',
            }
            try:
                r = req.head(actual_url, headers=headers, allow_redirects=True, verify=False, timeout=15)
                if r.status_code == 200:
                    api_call = r.url
                else:
                    api_call = actual_url
            except Exception as e:
                VSlog('downet.net request error: ' + str(e))
                api_call = actual_url
