...
class cHosterGui:
    ...
    def checkHoster(self, sHosterUrl, debrid=True):
        ...
        # lien direct ?
        if any(sHosterUrl.endswith(x) for x in ['.mp4', '.avi', '.flv', '.m3u8', '.webm', '.mkv', '.mpd', '.txt']):
        ...
        uppom = next((x for x in ['upbaam', 'upbam', 'uppom', 'uppboom', 'uupbom', 'upgobom', 'upptobom', 'up2b9om', 'up1bom', 'up3bom', 'u1pb3m', 
                            ...
                            'video15.shop', 'streaming15.shop', 'onlin12estream', 'tostream20', 'streaming200', 'top15top', 'uppbom', 'upbom'] if x in sHostName), None)
        if uppom:
        ...
        # Filemoon Clone
        filemoon = next((x for x in ['filemoon', 'moonmov', 'allviid', 'all-vid', 'techradar', 'albrq', 'kerapoxy', 'bysesukior'] if x in sHostName), None)
        if filemoon:
        ...
        if ('luluvdo' in sHostName):
            return self.getHoster('luluvdo')

        if ('share4max' in sHostName):
            return self.getHoster('share4max')

        if ('minochinos' in sHostName):
            return self.getHoster('minochinos')

        if ('cybervynx' in sHostName):
            return self.getHoster('cybervynx')

        if ('peytonepre' in sHostName):
        ...
        if ('megaupload.' in sHostName) or ('fansubs' in sHostName) or ('us.archive.' in sHostName) or ('ddsdd' in sHostName) or ('ffsff' in sHostName) or ('rrsrr' in sHostName)or ('fbcdn.net' in sHostName) or ('blogspot.com' in sHostName) or ('videodelivery' in sHostName) or ('bittube' in sHostName) or ('amazonaws.com' in sHostName) or ('top.mycima.cv' in sHostName):
            return self.getHoster('lien_direct')

        if ('akhbarworld.online' in sHostName):
            return self.getHoster('mycima')
