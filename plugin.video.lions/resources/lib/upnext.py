...
from resources.lib.comaddon import dialog, addon, addonManager, VSlog, isLions
...
class UpNext:
    ...
    def nextEpisode(self, guiElement):
        ...
        try:
            ...
            sParams = oOutputParameterHandler.getParameterAsUri()
            url = 'plugin://plugin.video.lions/?site=cHosterGui&function=play&%s' % sParams
    ...
    def notifyUpnext(self, data):
        ...
        try:
            ...
            jsonrpc_request = {
                ...
                "params": {
                    "sender": "%s.SIGNAL" % 'plugin.video.lions',
                    "message": 'upnext_data',
                    "data": [data],
                }
            }
