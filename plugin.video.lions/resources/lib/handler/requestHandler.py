...
from resources.lib.comaddon import addon, dialog, VSlog, VSPath, isLions
    ...
    def __callRequest(self, jsonDecode=False):
        ...
        try:
            ...
            if jsonDecode == True:
                ...
                if isLions() and 'youtube' not in self.oResponse.url:
                    try:
        ...
        if self.oResponse is not None:
            ...
            # Detect Cloudflare challenge on 200 responses (e.g. AJAX POST)
            if self.oResponse and self.oResponse.status_code == 200 and sContent:
                cfMarkers = ['Just a moment', 'Enable JavaScript and cookies to continue', 'Checking your browser', 'Verifying you are human']
                if any(marker in sContent for marker in cfMarkers):
                    bypass = addon().getSetting('cloudbypass')
                    if bypass in ('0', ''):
                        CLOUDPROXY_ENDPOINT = 'http://' + addon().getSetting('ipaddress') + ':8191/v1'
                        if addon().getSetting('Public_Flaresolverr') == "true":
